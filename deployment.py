...
final  = joblib.load('bankruptcy dataset.pkl')
label1 = joblib.load('le.pkl')
label2 = joblib.load('le1.pkl')
label3 = joblib.load('le2.pkl')
label4 = joblib.load('le3.pkl')
stand  = joblib.load('ss.pkl')
...
cash_flow          = st.number_input("Cash Flow", value=0.0, step=1000.0, format="%.2f")
...
if st.button("Predict Bankruptcy Risk"):
    features = np.array([[
        ...
        current_ratio,
        cash_flow,
        credit_rating_map[credit_rating],
        employee_count,
        market_trend_map[market_trend],
    ]])
 
    features_scaled = stand.transform(features)
    prediction = final.predict(features_scaled)[0]
